...
public class ItemControllerTest {
    ...
    public void getItemById(){
        webTestClient.get().uri(ITEM_END_POINT_V1.concat("/ABC"))
                .exchange()
                .expectStatus().isOk()
                .expectBody()
    ...
    public void updateItemInvalidId(){
        double newPrice = 1999.0;
        Item item = new Item("1", "Vivo v11", newPrice);
        webTestClient.put().uri(ITEM_END_POINT_V1.concat("/{id}"),item.getId())
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .body(Mono.just(item).log(),Item.class)
                .exchange()
                .expectStatus()
                .isNotFound();
    }
